...
const containerStyle: Record<string, string> = {
  primary: "flex w-fit bg-primary-bg p-1 rounded-xl gap-1 items-center",
  auth: "flex justify-center w-full gap-6",
};

const tabStyle: Record<string, string> = {
  primary: `relative px-4 py-1.5 text-ui-label text-muted border border-primary-bg
...
const activeStyle: Record<string, string> = {
  primary: "bg-secondary-bg text-txt-primary shadow-md",
  auth: "text-primary",
};

const inactiveStyle: Record<string, string> = {
  primary: "text-gray-500 hover:secondary-bg",
  ...
  >
    ...
    >
      ...
      <div
        v-if="variant === 'auth'"
        :class="[
          'absolute bottom-0 h-0.75 rounded-t-full transition-all duration-300',
          tab.id === modelValue
